...
from bot.services import key_service, ai_service, backend_health
...
from bot.utils.formatters import (
    ...
    format_backend_status, progress_bar, section,
)
...
async def genkey_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not args:
        ...
        await update.message.reply_text(
            ...
            f"Types: {types}\n\n"
            f"💡 <code>/genkey 1M promo</code>",
            parse_mode="HTML",
        )
        return
    key_type = args[0].upper()
    if not validate_key_type(key_type):
        await update.message.reply_text(f"❌ Types: {' '.join(KEY_TYPES.keys())}", parse_mode="HTML")
        return
...
async def genkeys_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(args) < 3:
        await update.message.reply_text(
            f"{E_KEY} <b>BULK GEN</b>\n{LINE}\n\n"
            f"<code>/genkeys TYPE QTY BATCH</code>\n\n"
            f"💡 <code>/genkeys 1M 50 jan</code>",
            parse_mode="HTML",
        )
        return
    key_type = args[0].upper()
    ...
    if not qty:
        await update.message.reply_text("❌ Qty: 1-500", parse_mode="HTML")
        return
    batch_name = validate_batch_name(args[2])
    if not batch_name:
        await update.message.reply_text("❌ Invalid batch", parse_mode="HTML")
        return
    loading = await update.message.reply_text(f"⏳ Gen {qty}...", parse_mode="HTML")
    try:
        ...
        file.name = f"{batch_name}_{key_type}_{qty}.txt"
        await loading.edit_text(f"{E_CHECK} <b>{qty} keys!</b>", parse_mode="HTML")
        await update.message.reply_document(file, filename=file.name, caption=f"{E_KEY} {qty}x {key_type}")
    except Exception as e:
        await loading.edit_text(f"❌ {e}", parse_mode="HTML")
...
async def keyinfo_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    args = context.args or []
    if not args:
        await update.message.reply_text("<code>/keyinfo KEY</code>", parse_mode="HTML")
        return
    ...
    try:
        ...
        if not info:
            ...
            return
        dot = {"UNUSED": "🟢", "USED": "🔵", "EXPIRED": "🟡", "REVOKED": "🔴"}.get(info["status"], "⚪")
        text = (
            f"{E_KEY} <b>KEY</b>\n{LINE}\n\n"
            f"<code>{info['key']}</code>\n"
            f"{info['key_type']} ({info['duration_days']}d)\n"
            f"{dot} {info['status']}\n"
            f"Batch: {info['batch_name'] or 'N/A'}"
        )
        if info.get("redeemed_at"):
            text += f"\n{LINE_LIGHT}\nBy: {info.get('redeemed_by_name', '?')}\nAt: {info['redeemed_at']}"
...
async def revokekey_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    args = context.args or []
    if not args:
        await update.message.reply_text("<code>/revokekey KEY</code>", parse_mode="HTML")
        return
    key_str = args[0].upper().strip()
    try:
        result = await key_service.revoke_key(update.effective_user.id, key_str)
        text = f"{E_CHECK} <b>REVOKED</b>\n<code>{result['key']}</code>"
        if result.get("downgraded_user"):
            text += f"\n⬇️ {result['downgraded_user']} → FREE"
        await update.message.reply_text(text, parse_mode="HTML")
    except CineBotError as e:
        await update.message.reply_text(e.user_message, parse_mode="HTML")
...
async def listkeys_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if not keys:
            await update.message.reply_text("No keys", parse_mode="HTML")
            return
        lines = [f"{E_KEY} <b>KEYS</b> ({total})\n{LINE}\n"]
        for k in keys:
            dot = {"UNUSED": "🟢", "USED": "🔵", "EXPIRED": "🟡", "REVOKED": "🔴"}.get(k.status.value, "⚪")
            lines.append(f"{dot} <code>{k.key[:15]}...</code> {k.key_type}")
...
async def userlookup_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    args = context.args or []
    if not args:
        await update.message.reply_text("<code>/userlookup ID</code>", parse_mode="HTML")
        return
    try:
...
async def giftkey_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(args) < 2:
        await update.message.reply_text(
            f"🎁 <b>GIFT</b>\n{LINE}\n\n"
            f"<code>/giftkey ID TYPE</code>\n"
            f"💡 <code>/giftkey 123 1M</code>",
            parse_mode="HTML",
        )
        return
    try:
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not args:
        await update.message.reply_text(
            ...
            f"<code>/broadcast [all|pro] msg</code>",
            parse_mode="HTML",
        )
        return
    target = args[0].lower()
    ...
    if not message_text:
        await update.message.reply_text("❌ No message", parse_mode="HTML")
        return
    loading = await update.message.reply_text("📡 Sending...", parse_mode="HTML")
    async with get_session() as session:
        user_ids = await (UserRepo.get_pro_user_ids if target == "pro" else UserRepo.get_all_user_ids)(session)
    sent, failed = 0, 0
    for uid in user_ids:
        try:
...
async def aistatus_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lines = [f"{E_ROBOT} <b>AI</b>\n{LINE}\n"]
    for name, info in status.items():
        if name == "_total":
            continue
        usage, limit, remaining = info["usage"], info["limit"], info["remaining"]
        bar = progress_bar(remaining, limit, 6)
        dot = "🔴" if info["exhausted"] else "🟡" if remaining < limit * 0.1 else "🟢"
        lines.append(f"{dot} <b>{name}</b>\n   {bar} {usage}/{limit}")
    total = status.get("_total", {})
    pct = (total.get("remaining", 0) / max(total.get("limit", 1), 1)) * 100
    lines.append(f"\n{LINE_LIGHT}\n⚡ {progress_bar(int(pct), 100)} {pct:.0f}%")
    await update.message.reply_text("\n".join(lines), parse_mode="HTML")
...
async def backend_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await ensure_user(update, context)
    await track_command(update, context)
    loading = await update.message.reply_text(f"{E_SERVER} Checking...", parse_mode="HTML")
    try:
...
async def admin_dashboard_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "adm:stats":
        ...
        await query.edit_message_text(
            f"{E_KEY} <b>GEN</b>\n{LINE}\n\n<code>/genkey TYPE [batch]</code>\nTypes: {' '.join(KEY_TYPES.keys())}",
            parse_mode="HTML",
        )
    elif data == "adm:bulkkeys":
        await query.edit_message_text(
            f"{E_KEY} <b>BULK</b>\n{LINE}\n\n<code>/genkeys TYPE QTY BATCH</code>",
            parse_mode="HTML",
        )
        ...
        await query.edit_message_text(
            f"{E_PERSON} <b>USER</b>\n{LINE}\n\n<code>/userlookup ID</code>",
            parse_mode="HTML",
        )
    elif data == "adm:broadcast":
        await query.edit_message_text(
            f"{E_SEND} <b>BROADCAST</b>\n{LINE}\n\n<code>/broadcast [all|pro] msg</code>",
            parse_mode="HTML",
        )
        ...
        await query.edit_message_text(
            f"{E_KEY} <b>LIST</b>\n{LINE}\n\n<code>/listkeys [STATUS] [page]</code>",
            parse_mode="HTML",
        )
