...
from bot.utils.constants import E_TV
...
async def where_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not query:
        await update.message.reply_text(
            ...
            f"Usage: <code>/where Movie Name</code>\n"
            f"Example: <code>/where Interstellar</code>",
            parse_mode="HTML",
        )
        return
    loading = await update.message.reply_text(f"{E_TV} Searching streaming options...", parse_mode="HTML")
    try:
        data = await tmdb_service.search_movies(query)
        results = data.get("results", [])
        if not results:
            await loading.edit_text("🔍 No movies found.", parse_mode="HTML")
            return
        movie = results[0]
        movie_id = movie["id"]
        ...
        text = f"🎬 <b>{title}</b> ({year})\n\n{format_streaming(info)}"
        if info and info.get("link"):
            text += f"\n\n🔗 <a href=\"{info['link']}\">More info</a>"
...
async def where_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.answer("Loading streaming info...")
