...
from bot.utils.formatters import format_movie_card, format_movie_credits
from bot.utils.keyboards import movie_detail_kb, search_results_kb
from bot.utils.validators import validate_movie_title
from bot.utils.constants import E_SEARCH, MSG_NO_RESULTS
...
async def search_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not query:
        await update.message.reply_text(
            f"{E_SEARCH} <b>Search for a movie:</b>\n\n"
    ...
    if not title:
        await update.message.reply_text("❌ Invalid search query. Please try again.", parse_mode="HTML")
        return
    ...
    loading = await update.message.reply_text(f"{E_SEARCH} Searching for '<b>{title}</b>'...", parse_mode="HTML")

    try:
        data = await tmdb_service.search_movies(title)
        results = data.get("results", [])
        if not results:
            await loading.edit_text(MSG_NO_RESULTS, parse_mode="HTML")
            return
        ...
        if len(results) == 1 or results[0].get("vote_count", 0) > 100:
            ...
            await loading.edit_text(
                f"{E_SEARCH} Found <b>{len(results[:8])}</b> results for '<b>{title}</b>':\n\nSelect a movie:",
...
async def _show_movie_detail(message, movie_id: int, user_db_id: int, edit: bool = True) -> None:
    try:
        ...
        if edit:
            ...
            if poster_url:
                try:
                    await message.reply_photo(poster_url, caption=card_text[:1024], reply_markup=kb, parse_mode="HTML")
                    return
                except Exception:
                    pass
            await message.reply_text(card_text, reply_markup=kb, parse_mode="HTML")
...
async def similar_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer("Loading similar movies...")
    movie_id = int(query.data.split(":")[1])
    try:
        ...
        await query.answer("Failed to load similar movies.", show_alert=True)
