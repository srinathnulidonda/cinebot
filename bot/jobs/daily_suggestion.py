...
async def daily_suggestion_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        pro_ids, free_ids = await _fetch_user_ids()
    except Exception as e:
        logger.error("Daily suggestion aborted — DB unreachable after retries: %s", e)
        return

    sample_size = min(len(free_ids), len(free_ids) // 5 + 1)
    all_ids = list(set(pro_ids + random.sample(free_ids, sample_size)))
    sent, failed = 0, 0

    for telegram_id in all_ids:
        try:
            user, prefs, watched_ids = await _fetch_user_data(telegram_id)
            if not user:
                continue
            ...
            if liked_genres:
                ...
                genre_ids = [
                    reverse_map[g] for g in user.preferred_genres if g in reverse_map
                ][:3]
